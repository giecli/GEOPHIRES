...
if resoption == 1 or resoption == 2:    #the first two reservoir models require fracture geometry
    ...
    try:
        ...
        fracshape = 1
        print("Warning: Provided fracture shape is not valid. GEOPHIRES will assume default fracture shape (1)")
    ...
    if fracshape == 1:
        ...
        fracarea = fracheight*fracheight
    elif fracshape == 4:
...
if resoption == 1:
    ...
    td = (rhowater*cpwater)**2/(krock*rhorock*cprock)*(q/fracnumb/fracwidth/fracheight)**2*timevector*365.*24.*3600
...
if impedancemodelused == 1: #assumed everything stays liquid throughout
    ...
    if productionwellpumping == 1:
        ...
        elif pumpdepthfinal > depth:
            print("Warning: GEOPHIRES calculates pump depth to be deeper than production well depth. Verify reservoir pressure, production well flow rate and production well dimensions")  
...
if ccstimfixedvalid == 1:
    ...
    Cstim = ccstimadjfactor*ninj*1.25 
...
if totalcapcostvalid == 0:  
    ...
    if ccexplfixedvalid == 1:
        ...
        Cexpl = ccexpladjfactor*1.12*(1. + C1well*0.6)
    ...
    if ccgathfixedvalid == 1:
        ...
        Cgath = ccgathadjfactor*1.12*((nprod+ninj)*750*500. + Cpumps)/1E6 #Based on GETEM 2016
    ...
    if enduseoption == 2: #direct-use
        ...
        if pptype == 1: #sub-critical ORC
            ...
            if (MaxProducedTemperature < 170.):
                ...
                CCAPP1 = 2212.5 - 0.514*(MaxProducedTemperature-170.) 
        ...
        if ccplantfixedvalid == 1:
            ...
            Cplant = ccplantadjfactor*Cplantcorrelation*1.02 #1.02 to convert cost from 2012 to 2016
        
        #add direct-use plant cost of co-gen system to Cplant (only of no total ccplant was provided)
        if ccplantfixedvalid == 0:
            if (math.floor(enduseoption/10) == 3): #enduseoption = 3: cogen topping cycle
                    Cplant = Cplant + ccplantadjfactor*150E-6*np.max(HeatProduced/enduseefficiencyfactor)*1000.
            elif (math.floor(enduseoption/10) == 4): #enduseoption = 3: cogen topping cycle
                    Cplant = Cplant + ccplantadjfactor*150E-6*np.max(HeatProduced/enduseefficiencyfactor)*1000.
            elif (math.floor(enduseoption/10) == 5): #enduseoption = 3: cogen topping cycle
                    Cplant = Cplant + ccplantadjfactor*150E-6*np.max(HeatProduced/enduseefficiencyfactor)*1000.
...
if printoutput == 1:
    ...
    print(" GEOPHIRES Build Date = 2018-06-06")
    currentdate = datetime.datetime.now().strftime("%Y-%m-%d")
    currenttime = datetime.datetime.now().strftime("%H:%M")
    print(" Simulation Date = "+ currentdate)
    print(" Simulation Time = "+ currenttime)
    print(" Calculation Time = "+"{0:.3f}".format((time.time()-t)) +" s")
    ...
    if printoutput == 1:
        ...
        if enduseoption == 1:
            ...
            print(" LCOE = " + "{0:.1f}".format((Price)) +" cents/kWh")
            print(" Additional average annual revenue from heat sales = " + "{0:.1f}".format((annualheatincome)) +" M$/year")
        elif enduseoption % 10 == 1: #electricity sales is additional income revenuw stream
            print(" LCOH = " + "{0:.1f}".format((Price)) +" $/MMBTU")
            print(" Additional average annual revenue from electricity sales = " + "{0:.1f}".format((annualelectricityincome)) +" M$/year")    
